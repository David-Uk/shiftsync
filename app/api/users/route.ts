...
export async function POST(req: NextRequest) {
  try {
    ...
    if (contentType && contentType.includes("multipart/form-data")) {
      ...
      const { fields, profileImageUrl: uploadedUrl } =
    ...
    const sanitizedData = sanitizeUserCreation({
      ...
      designation: typeof userData.designation === "string" ? userData.designation : undefined,
    });
      ...
      try {
        ...
        if (!existingStaff) {
          const staffRecord = new Staff({
            user: newUser._id,
            designation: sanitizedData.designation || 
              (sanitizedData.role === "manager" ? "Manager" : 
              (sanitizedData.role === "staff" ? "Staff Member" : "Employee")),
            status: "active",
          });
    ...
    try {
      await NotificationService.createAdminNotification({
        ...
        relatedEntity: {
          type: "user",
          id: newUser._id as mongoose.Types.ObjectId,
        },
        priority: "medium",
      ...
      console.error(
        "Failed to send user creation notification:",
        notificationError,
      );
    }
