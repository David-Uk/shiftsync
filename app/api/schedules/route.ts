...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      !isOneOff &&
      (!workDays || !Array.isArray(workDays) || workDays.length === 0)
    ) {
      return NextResponse.json(
        {
          success: false,
          error: "Recurring schedules require at least one work day",
        },
        { status: 400 },
      );
    }
    ...
    // Get existing schedules for this staff member to check for conflicts
    const existingSchedules = await Schedule.find({
      staff: staffToSchedulesFor._id,
    });

    // Check for duplicate work days in recurring schedules
    if (!isOneOff && workDays && workDays.length > 0) {
      const duplicateDays: string[] = [];

      for (const newDay of workDays) {
        for (const existingSchedule of existingSchedules) {
          if (
            !existingSchedule.isOneOff &&
            existingSchedule.workDays.includes(newDay)
          ) {
      ...
      if (duplicateDays.length > 0) {
        return NextResponse.json(
          {
            success: false,
            error: `The following days are already scheduled: ${duplicateDays.join(", ")}. Please select different work days.`,
          },
          { status: 400 },
        );
      }
    ...
    // Check for one-off date conflicts with existing work days
    if (isOneOff && oneOffDate) {
      const oneOffDateObj = new Date(oneOffDate);
      const dayOfWeek = [
        "Sunday",
        "Monday",
        "Tuesday",
        ...
        "Friday",
        "Saturday",
      ][oneOffDateObj.getUTCDay()];

      // Check if this day of week is already scheduled in any recurring schedule
      const conflictingSchedules = existingSchedules.filter(
        (schedule) =>
      ...
      if (conflictingSchedules.length > 0) {
        return NextResponse.json(
          {
            success: false,
            error: `The date ${new Date(oneOffDate).toLocaleDateString()} falls on a ${dayOfWeek} which is already scheduled as a recurring work day. Please choose a different date.`,
          },
          { status: 400 },
        );
      }
