...
const ScheduleSchema: Schema<ISchedule> = new Schema(
  {
    ...
    oneOffDate: {
      ...
      validate: {
        ...
        message:
          "oneOffDate is required for one-off schedules and should not be provided for recurring schedules",
      },
    },
    timezone: {
      type: String,
...
ScheduleSchema.pre("save", async function (this: ISchedule) {
  if (
    ...
    this.isModified("workDays") ||
    this.isModified("oneOffDate") ||
    ...
    try {
      ...
      for (const other of existingSchedules) {
        ...
        if (this.isOneOff && other.isOneOff) {
          ...
          const thisLocalStart = convertToTimezone(
            this.startTime,
            this.timezone,
          );
          const thisLocalEnd = convertToTimezone(this.endTime, this.timezone);
          const otherLocalStart = convertToTimezone(
            other.startTime,
            other.timezone,
          );
          const otherLocalEnd = convertToTimezone(
            other.endTime,
            other.timezone,
          );

          overlap =
            thisLocalStart < otherLocalEnd && thisLocalEnd > otherLocalStart;
        } else if (!this.isOneOff && !other.isOneOff) {
          // Both recurring: Check if any day overlaps (timezone-agnostic for recurring)
          const commonDays = this.workDays.filter((day) =>
            other.workDays.includes(day),
          );
          ...
          const oneOffInRecurringTz = convertToTimezone(
            oneOff.startTime,
            recurring.timezone,
          );
          const oneOffDay = [
          ...
          if (recurring.workDays && recurring.workDays.includes(oneOffDay)) {
            ...
            const oneOffEndInRecTz = normalizeToTimeOnly(
              convertToTimezone(oneOff.endTime, recurring.timezone),
            );
            const recStart = normalizeToTimeOnly(recurring.startTime);
            const recEnd = normalizeToTimeOnly(recurring.endTime);

            overlap =
              oneOffStartInRecTz < recEnd && oneOffEndInRecTz > recStart;
          }
        }
        ...
        if (overlap) {
          ...
          throw new Error(
            `${scheduleType} schedule conflicts with existing ${otherType} schedule. Schedules cannot overlap across different timezones.`,
          );
        }
        ...
        if (this.isOneOff) {
          ...
          if (other.isOneOff) {
            ...
            const gap1 =
              (thisLocalStart.getTime() - otherLocalEnd.getTime()) /
              (1000 * 60 * 60);
            const gap2 =
              (otherLocalStart.getTime() - thisLocalEnd.getTime()) /
              (1000 * 60 * 60);
            ...
            for (const offset of daysToCheck) {
              ...
              const checkDateInRecTz = convertToTimezone(
                checkDate,
                other.timezone,
              );
              const dayName = [
              ...
              if (other.workDays && other.workDays.includes(dayName)) {
                ...
                recStart.setHours(
                  other.startTime.getHours(),
                  other.startTime.getMinutes(),
                  0,
                  0,
                );
                ...
                recEnd.setHours(
                  other.endTime.getHours(),
                  other.endTime.getMinutes(),
                  0,
                  0,
                );
                ...
                const recStartInOneOffTz = convertToTimezone(
                  recStart,
                  this.timezone,
                );
                const recEndInOneOffTz = convertToTimezone(
                  recEnd,
                  this.timezone,
                );
                const oneOffStart = convertToTimezone(
                  this.startTime,
                  this.timezone,
                );
                const oneOffEnd = convertToTimezone(
                  this.endTime,
                  this.timezone,
                );

                const gap1 =
                  (oneOffStart.getTime() - recEndInOneOffTz.getTime()) /
                  (1000 * 60 * 60);
                const gap2 =
                  (recStartInOneOffTz.getTime() - oneOffEnd.getTime()) /
                  (1000 * 60 * 60);
          ...
          if (gapError) {
            throw new Error(
              `10-hour rest rule violation: Only ${diffHours.toFixed(1)} hours rest between shifts across timezones.`,
            );
          }
        }
      }
...
ScheduleSchema.statics.toLocalTime = function (
  utcDate: Date,
  timezone: string,
): Date {
...
ScheduleSchema.statics.toUTC = function (
  localDate: Date,
  timezone: string,
): Date {
  const timezoneMap: Record<string, string> = {
...
ScheduleSchema.methods.toLocalSchedule = function (
  this: ISchedule,
  ...
  return {
    ...
    oneOffDate: this.oneOffDate
      ? ScheduleModel.toLocalTime(this.oneOffDate, this.timezone)
      : undefined,
  };
};
...
const Schedule =
  ...
  mongoose.model<ISchedule, IScheduleModel>("Schedule", ScheduleSchema);
